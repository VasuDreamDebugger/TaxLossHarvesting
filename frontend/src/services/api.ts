...
export interface RawCapitalGainsResponse {
  capitalGains: {
    stcg: {
      profits: number;
      losses: number;
...
const HOLDINGS_DATA: RawHolding[] = [
  {
    "coin": "USDC",
    "coinName": "USDC",
...
const CAPITAL_GAINS_DATA: RawCapitalGainsResponse = {
  capitalGains: {
    stcg: {
      profits: 70200.88,
      losses: 1548.53
...
/**
 * Fetch holdings from the mock API and transform them into the application's Asset type.
 */
export const fetchHoldings = async (): Promise<Asset[]> => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      try {
        // Transform the raw API data to match our application's UI model
        const assets: Asset[] = HOLDINGS_DATA.map((item) => ({
          id: item.coin,
          symbol: item.coin,
          name: item.coinName,
          shares: item.totalHolding,
...
/**
 * Fetch base capital gains from the mock API and transform them into the application's CapitalGains type.
 */
  ...
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      try {
        ...
        reject(new Error("Failed to parse capital gains data"));
      }
    }, 500);
  });
};
